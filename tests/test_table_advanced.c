...
void test_m_table_introspection_comprehensive ()
{
	...
	assert (m_table_get_type_cstr (table, "c") ==
	...
	assert (m_table_get_type_cstr (table, "non-existent") ==
...
void test_m_table_get_with_defaults ()
{
	...
	// Current m_table API doesn't have "get with default" in its basic
	// form, but we can test behavior of non-existent keys.
