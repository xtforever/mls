	...
	*sec = (section_t){0};
...

	{
		...
		*t = (table_t){0};
		...
		r = kv_pair("OS", u.sysname); m_put(t->rows, &r);
		r = kv_pair("Kernel", u.release); m_put(t->rows, &r);
		r = kv_pair("Arch", u.machine); m_put(t->rows, &r);
	...
	if (cpuinfo) {
		...
		*t = (table_t){0};
		...
		t->rows = m_create(4, sizeof(int));
		int r;
		r = kv_pair("Processors", buf); m_put(t->rows, &r);

		{
			...
			if (hit) { hit = strchr(hit, ':'); hit = hit ? hit + 1 : NULL;
				...
				if (hit) { const char *nl = strchr(hit, '\n');
					...
					model = free_me; } }
			r = kv_pair("Model", model); m_put(t->rows, &r);
			free(free_me);
		}
		...
		if (getloadavg(av, 3) == 3) {
			snprintf(buf, sizeof(buf), "%.2f / %.2f / %.2f (1/5/15 min)",
				 av[0], av[1], av[2]);
			r = kv_pair("Load", buf); m_put(t->rows, &r);
		}
