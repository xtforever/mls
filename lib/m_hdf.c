...
static const char *parse_raw_string(const char *p, int *node) {
    ...
    while (*p) {
        if (*p == ']') {
            ...
            p++;
            int end_equals = 0;
            while (*p == '=') { end_equals++; p++; }
            if (*p == ']' && end_equals == equals) {
                int tmp = s_strdup_c(start);
                int s_h = m_alloc(0, 1, MFREE);
                s_slice(s_h, 0, tmp, 0, (end_cand - start) - 1);
                // No easy way to free tmp if it's a conststr, but s_strdup_c in m_tool.h might return a regular handle.
                // Looking at m_tool.h: s_strdup_c(const char *s)
...
static const char *parse_quoted_string(const char *p, int *node) {
    ...
    while (*p && *p != '"') {
        if (*p == '\\') {
            ...
            switch (*p) {
                case 'n': m_putc(s_h, '\n'); break;
                case 'r': m_putc(s_h, '\r'); break;
                case 't': m_putc(s_h, '\t'); break;
                case '\\': m_putc(s_h, '\\'); break;
                case '"': m_putc(s_h, '"'); break;
                default: m_putc(s_h, *p); break;
            }
        } else {
            m_putc(s_h, *p);
        }
...
static int is_keyword_char(char c, int initial) {
    ...
    if (!initial && (isdigit(c) || strchr(".:/-", c))) return 1;
    return 0;
}

static const char *parse_keyword_or_literal(const char *p, int *node) {
    const char *start = p;
    while (*p && is_keyword_char(*p, 0)) p++;
...
static const char *parse_element(const char *p, int *node) {
    p = skip_ws(p);
    if (*p == '(') return parse_list(p, node);
    if (*p == '"') return parse_quoted_string(p, node);
    if (*p == '[') return parse_raw_string(p, node);
    if (is_keyword_char(*p, 1) || *p == '-' || isdigit(*p)) return parse_keyword_or_literal(p, node);
    return NULL;
}
...
const char *hdf_get_value(int h) {
    ...
    if (type == HDF_TYPE_LIST) return NULL;
    int val = INT(h, 1);
    return val > 0 ? m_str(val) : NULL;
}

int hdf_get_children(int h) {
    if (hdf_get_type(h) != HDF_TYPE_LIST) return -1;
    return INT(h, 1);
}
    ...
    for (int i = 0; i < m_len(children); i++) {
        ...
        if (hdf_get_type(child) == HDF_TYPE_LIST) {
            ...
            if (m_len(sub_children) > 0) {
                ...
                if (val && strcmp(val, key) == 0) return child;
            }
        }
    }
    return -1;
...
const char *hdf_get_property(int h, const char *key) {
    ...
    if (m_len(children) < 2) return NULL;
