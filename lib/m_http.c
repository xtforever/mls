...
static int parse_response(int data_h, int start, int end, int *version, int *status_code, int *reason) {
    int line_h = m_slice(0, 0, data_h, start, end);
    int parts = m_alloc(0, sizeof(char*), MFREE_STR);
    m_str_split(parts, m_str(line_h), " ", 1);
...
int http_parse(http_parser_t *p, int data_h) {
    ...
    while (pos < data_len && p->state != HTTP_STATE_DONE && p->state != HTTP_STATE_ERROR) {
        ...
        if (p->state == HTTP_STATE_REQUEST_LINE) {
            ...
            int reason;
            if (!parse_response(data_h, pos, crlf - 1, &p->version, &p->status_code, &reason)) {
                set_error(p, HTTP_ERROR_INVALID_STATUS);
                return -1;
            }
            if (reason > 0) m_free(reason);
            ...
            if (p->is_chunked) {
                int new_pos = parse_body_chunked(p, data_h, pos, data_len);
